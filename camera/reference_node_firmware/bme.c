...
#define SENSORDATA_NOTEFILE         "*#camera001.qo"

#define CHRIS_CAMERA_PIN GPIO_PIN_2
#define CHRIS_CAMERA_GPIO_PORT GPIOB

// TRUE if we've successfully registered the template
static bool templateRegistered = false;
static GPIO_PinState cameraReading;
static GPIO_PinState noFloodReading;
static double floodStatus = -1;
//cameraReading.GPIO_PIN_SET = 0;
...
bool bmeInit()
{
    ...
    schedAppConfig config = {
        .name = "bme",
        //.activationPeriodSecs = 60 * 6,
		.activationPeriodSecs = 15,
        .pollPeriodSecs = 15,
        .activateFn = NULL,
        .interruptFn = NULL,
        .pollFn = bmePoll,
...
static bool registerNotefileTemplate()
{
    ...
    JAddNumberToObject(body, "floodstatus", TFLOAT16);
...
static bool addNote()
{
	// Measure the camera value
	//cameraReading = HAL_GPIO_ReadPin(CHRIS_CAMERA_GPIO_PORT, CHRIS_CAMERA_PIN);
	cameraReading = HAL_GPIO_ReadPin(LED_PAIR_GPIO_Port, LED_PAIR_Pin);
	noFloodReading = HAL_GPIO_ReadPin(LED_RX_GPIO_Port, LED_RX_Pin);
	if (cameraReading) {
		floodStatus = 2; // FLood
	...
	else if (noFloodReading){
		floodStatus = 1; // No Flood
		//HAL_GPIO_WritePin(LED_RX_GPIO_Port, LED_RX_Pin, LED_RX_ON);
		//HAL_GPIO_WritePin(LED_PAIR_GPIO_Port, LED_PAIR_Pin, LED_PAIR_ON);
		//HAL_GPIO_WritePin(LED_TX_GPIO_Port, LED_TX_Pin, LED_TX_ON);
	}
	else if (!cameraReading && !noFloodReading){
		floodStatus = -1; // Invalid Reading
		// COMMENTED BELOW OUT FOR DEBUGGING \/\/\/\/
		return false; // If invalid reading, do not send to Notehub... I think
	}
	//printf("%d",floodStatus);
    ...
    // Fill-in the body
    JAddNumberToObject(body, "floodstatus", floodStatus);
